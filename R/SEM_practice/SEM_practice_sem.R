...
#####
cnt = 0
checklist <- c(3, 4, 7, 8, 9, 10, 11, 12, 13, 14, 15)
for (i in 1:1000){ #simulating 1000 datasets 
  data <- simulateData(pop_model, sample.nobs = 350) # each dataset contains 500 participants
  fit <- sem(model = my_model, data=data, fixed.x=F) # fit the model to each dataset 
  result <- parameterEstimates(fit) # save the row for y ~ x2 for each dataset 
  for (i in checklist){
    if (result$pvalue[i] >= .05){
      cnt = cnt + 1
      break
    } #탈락 카운트.
  }
}
print(cnt/1000) #10000, 280 obs. factor loading & regression path -> 0.0499
...
for(j in seq(300,350,by=10)){ # trying sample sizes between 150 to 250, in increments of 10 
  results <- NULL 
  for (i in 1:1000){ # now simulating 1000 datasets for each sample size 
    data <- simulateData(pop_model, sample.nobs = j) # j corresponds to the sample size 
    fit <- sem(model = my_model, data=data, fixed.x=F) 
    results <- rbind(results,parameterEstimates(fit)) # row for y ~ x2 
    powerlist[[j]] <- mean(results$pvalue < .05) 
  }
}
